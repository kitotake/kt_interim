...
RegisterNetEvent('kt_interim:depositItems', function(jobType, itemName, itemAmount, reward)
    ...
    if not identifier then
        ServerUtils.Log('Could not get player identifier', 'ERROR', source)
        return
    end
    ...
    if rewarded then
        ...
        ServerUtils.Log(string.format('Job completed: %s | Reward: $%d | Items: %d', 
            jobType, reward, itemAmount), 'SUCCESS', source)
    ...
    if args[1] then
        ...
        if ServerUtils.PlayerExists(targetSource) then
            if activeJobs[targetSource] then
                ...
                print(('^2[KT_INTERIM]^7 Force stopped job for player: %s (Job: %s)'):format(GetPlayerName(targetSource), jobType))
            else
                print('^1[KT_INTERIM]^7 Player has no active job')
            end
        else
...
-- Cleanup lors de l'arrêt de la ressource
AddEventHandler('onResourceStop', function(resourceName)
    if GetCurrentResourceName() == resourceName then
        print('^3[KT_INTERIM]^7 Resource stopping - Cleaning up active jobs')
        
        -- Notifier tous les joueurs avec un job actif
        for playerId, jobType in pairs(activeJobs) do
            if ServerUtils.PlayerExists(playerId) then
...
-- Réinitialisation lors du démarrage
