...
function StartConstructionJob(config)
    ...
    if config.vehicleSpawn then
        ClientUtils.SpawnVehicle(config.vehicleSpawn.model, config.vehicleSpawn.coords, config.vehicleSpawn.coords.w,
            function(vehicle)
                if vehicle then
                    jobVehicle = vehicle
    ...
    local threadId = CreateThread(function()
        while IsJobActive() and collectedItems < config.item.amount do
            ...
            if distance < 20.0 then
                ClientUtils.DrawMarker(config.collectPoint.coords, config.collectPoint.markerType,
...
function StartConstructionDeposit(config)
    ...
    local threadId = CreateThread(function()
        while IsJobActive() do
            ...
            if distance < 20.0 then
                ClientUtils.DrawMarker(config.depositPoint.coords, config.depositPoint.markerType,
...
function DeliverPackage(config)
    ...
    if success then
        ...
        local alert = lib.alertDialog({
            header = 'Continuer le travail ?',
            content =
            'Voulez-vous livrer un autre colis ?\n\n✅ Oui - Retourner au dépôt\n❌ Non - Terminer et rendre le véhicule',
            centered = true,
            cancel = true,
            labels = {
                confirm = 'Oui, continuer',
...
function DepositShopBoxes(config)
    ...
    if success then
        ...
        local alert = lib.alertDialog({
            header = 'Continuer le travail ?',
            content =
            'Voulez-vous transporter plus de cartons ?\n\n✅ Oui - Retourner au point de collecte\n❌ Non - Terminer et rendre le véhicule',
            centered = true,
            cancel = true,
            labels = {
                confirm = 'Oui, continuer',
            ...
            if distance < 20.0 then
                ...
                if distance < 5.0 and ClientUtils.IsInVehicle() then
                    ...
                    local alert = lib.alertDialog({
                        header = 'Continuer le travail ?',
                        content =
                        'Voulez-vous prendre un autre client ?\n\n✅ Oui - Chercher un nouveau client\n❌ Non - Terminer et rendre le véhicule',
                        centered = true,
                        cancel = true,
                        labels = {
                            confirm = 'Oui, continuer',
        ...
        function(truck)
            if truck then
                ...
                if config.vehicleSpawn.trailer then
                    ...
                    jobTrailer = ClientUtils.SpawnTrailer(config.vehicleSpawn.trailer, trailerCoords,
